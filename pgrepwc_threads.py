...
def criarFilhos(n, filesList, option, name):
    ...
    if option == "-c":
        ...
        for i in range(len(filesList)):
            ThreadList.append(Thread(target = contarPalavras, args=(name, filesList[i])))
                   

    elif option == "-l":
        
        for i in range(len(filesList)):
            ThreadList.append(Thread(target = contarLinhas, args=(name, filesList[i])))
...
print("== Pai ==\nO total deu: " + str(contador.value))
